...
{	
	...
	private function _configMargin()
	{
	    ...
	    {	
	    	...
	    	if($this->maxPeriod == 3)
	    	{
	    		...
	    		// Para los que tiene 2 desempeño sin AEE
	    		else if(count($this->evaluation_parameters) == 2)
	    		{
	    			$this->_with_C_H = 58;
		    		$this->_with_C_S = 91;
	    		}
	...
	private function _header()
	{
		...
		// 
		$this->Cell( ( ($this->_with_C_N_E * 2 ) + ($this->_with_period * $this->maxPeriod) + $this->_with_C_S) , 4, "Periodo {$this->current_period}", 1,0, 'C', true);
		...
		foreach ($data as $key => $value) {
			...
			if($value['alu_primer_nom'] != NULL){
				...
				if($key < 9)
				{
					$this->Cell($this->_with_C_S, 4, '0'.($key+1).' '.
						substr(
							...
							$value['alu_segundo_nom'], 0,37), 1,0, 'L', false);
				}
				else
				{
				...
				for ($i=0; $i < $this->maxPeriod; $i++) {
					...
					if($nota == NULL || $nota == 0 || ($i+1) > $this->current_period)
